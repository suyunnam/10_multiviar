...
If wanted to use enough PCs to explain 70% of total variance, how many would we need? 3
...
Which variables contributed most to PC1, regardless of direction?  daylength of colder months
...
vif(lm_pca)
```
All vif is 1, because all PCs are orthogonal and independent
...
Any thoughts?  
Let's try 6. (based on PCA score scatter plot)  
...
Find the elbow or break point. It is a trade off. Too many clusters do not give use useful information. But too little clusters will have too high variance and not segregate data enough. 3-5 clusters looks fine from this plot
...
- add the cluster column to original data set and explore the distribution of each variable against cluster id, OR  

- use a function that summarizes all the original variables into PCs and plots the cluster ids.  
